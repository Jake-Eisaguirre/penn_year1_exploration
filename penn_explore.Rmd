...
#### VES wrangling - Tuttle Pond and Wood Lab Pond sites Repeated
...
ggplot(data = comb_data, aes(x = fct_infreq(species), y = scaled_count, fill = capture_ves)) +
  ...
  ylab("Percent Occurrence") +
  xlab("Species") +
  labs(title = "Capture and VES Species Counts of Matching Sites and Species",
       subtitle = "")
...
### Wrangle Capture/VES Analysis Data

```{r}

ggplot(data = comb_data, aes(x = fct_infreq(site), y = scaled_count, fill = capture_ves)) +
  geom_col(stat = "identity", position = position_dodge()) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5),
        strip.background = element_blank()) +
