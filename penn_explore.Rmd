...
#### Tukey post-hoc test to determine significant date
```{r}

date_tukey <- TukeyHSD(one_way_date)

tidy(date_tukey) %>% 
  kable(digits = 2) %>% 
  kable_styling()

plot(date_tukey, las = 3, cex.axis = 0.5)
...
spec_tukey <- TukeyHSD(one_way_spec)

tidy(spec_tukey) %>% 
  kable(digits = 2) %>% 
  kable_styling()

plot(spec_tukey, las = 3, cex.axis = 0.3)
